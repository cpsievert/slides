...
* Prefix any function with `?` to read documentation.

```{r, eval = FALSE}
?nrow
```

* Reading source code is also a great way to learn `R`!

```{r}
nrow
```

* But what about _finding_ function(s) to help solve a problem???

---

* Google! (I usually prefix "CRAN" to my search; others might suggest <http://www.rseek.org/>)

* Ask your question on <http://stackoverflow.com/> (below is the # of questions asked per month)

<img src="so-raw.png" width="800" height="450" />

---

* Chances are, someone has authored a package to help solve your problem.

* After installing a package, check to see if it has any vignettes explaining it's functionality (__dplyr__ has _awesome_ vignettes):
...
```{r, cache = FALSE}
# this runs your turn solution script
...
plotly::plot_ly(hc, x = year, y = prop, color = sex, colors = c("blue", "hotpink"))
```
